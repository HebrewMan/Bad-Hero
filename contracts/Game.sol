...
contract Game is AccessControl, Ownable {
    ...
    gameInfo public _gameInfo =
        gameInfo(12 * 3600, 5, 20000 * 10**18, 100, 10, 25, 2000 * 10**18);
    ...
    function setupMonsterFee(uint256 _fee) external onlyOwner {
        upMonsterFee = _fee;
    }
     ...
     * @prames _rate 比例
     */
    function setReceiveRate(uint256 _rate) external onlyOwner {
    ...
    //set monster type
    function setTokenDetailGenre(uint256 tokenId, uint32 genre)
        public
    ...
    function createCard(
        ...
        CardDetails memory _carDetails = CardDetails(
            0,
            tokenId,
            basicHp,
            1,
    ...
    function adRecruit(uint256 tokenId) public {
        ...
        require(
            _carDetail.unLockTime > block.timestamp,
    ...
    function moveToBack(uint256 tokenId) public {
        require(_tokenUser[tokenId] == msg.sender, "Have no legal power");
        require(
            _userTem[msg.sender].contains(tokenId) == true,
    ...
    function moveToTem(uint256 tokenId) public {
        require(_tokenUser[tokenId] == msg.sender, "Have no legal power");
        require(
            _userBackpack[msg.sender].contains(tokenId) == true,
            "It's already decompressed"
        );
    ...
    function moveBack(uint256 tokenId, address sender)
        public
        onlyRole(MINTER_ROLE)
    {
        require(_tokenUser[tokenId] == sender, "Have no legal power");
        require(
            _userBackpack[sender].contains(tokenId) == true,
            "It's already decompressed"
        );
    ...
    function addBack(uint256 tokenId, address sender)
        public
        onlyRole(MINTER_ROLE)
    {
        require(_tokenUser[tokenId] == sender, "Have no legal power");
        require(
            _userBackpack[sender].contains(tokenId) == false,
            "It's already decompressed"
        );
    ...
    function speedFee(uint256 remainTime) public view returns (uint256) {
        if (remainTime <= 0) {
            return 0;
        }
        ...
        if (addType == 1) {
            if (userBnbPool[user].isVaild) {
                if (userBnbPool[user].reward == 0) {
                    userBnbPool[user].validTime =
                        block.timestamp +
                        _receiveInfo.lockTime;
                    userBnbPool[user].unLockTime =
                        block.timestamp +
                        _receiveInfo.lockTime +
                        _receiveInfo.freeDay *
                        _unlockTime;
                }
            } else {
                userBnbPool[user].validTime =
            ...
            if (userTokenPool[user].isVaild) {
                if (userTokenPool[user].reward == 0) {
                    userTokenPool[user].validTime =
                        block.timestamp +
                        _receiveInfo.lockTime;
                    userTokenPool[user].unLockTime =
                        block.timestamp +
                        _receiveInfo.lockTime +
                        _receiveInfo.freeDay *
                        _unlockTime;
    ...
    function addTokenReward(address user, uint256 reward)
        public
        onlyRole(MINTER_ROLE)
    {
    ...
    function fighting(uint256 tokenId, uint256 enemyId)
        public
        isUnlock(tokenId)
        isTeam(tokenId, msg.sender)
        returns (FightingEndInfo memory fig)
    {
        (fig.suc, fig.reward, fig.hp, fig.xp, fig.unLkTime) = _monster.fighting(
            tokenId,
            enemyId,
            msg.sender
        );
    ...
    function DoTask(uint256 tokenId, uint256 enemyId)
        public
        isUnlock(tokenId)
        isTeam(tokenId, msg.sender)
        returns (FightingEndInfo memory fig)
    {
        ...
        (fig.suc, fig.reward, fig.hp, fig.unLkTime) = _monster.DoTask(
            tokenId,
            _task.odds,
            _task.basicReward,
            msg.sender
    ...
    function DisReward(address rewardAddr, uint256 reward)
        public
        onlyRole(MINTER_ROLE)
    {
    ...
    function upLevel(uint256 _tokenId) public {
        ...
        require(
            tokenDetail[_tokenId].level < _gameInfo.maxLevel,
            "_gameInfo.maxLevel"
        );
    ...
    function drawReward(uint256 index) public returns (bool) {
        ...
        if (index == 1) {
            require(
                userBnbPool[msg.sender].validTime < block.timestamp,
                "The unlock time is not reached"
            );
            if (userBnbPool[msg.sender].unLockTime <= block.timestamp) {
                rallReward = userBnbPool[msg.sender].reward;
            } else {
                rateFee = getFee(
                    userBnbPool[msg.sender].unLockTime - block.timestamp
                );
                rallReward =
                    userBnbPool[msg.sender].reward -
                    (userBnbPool[msg.sender].reward * rateFee) /
                    100;
            }
            ...
            require(
                userTokenPool[msg.sender].validTime < block.timestamp,
                "The unlock time is not reached"
            );
            if (userTokenPool[msg.sender].unLockTime <= block.timestamp) {
                ...
                rateFee = getFee(
                    userTokenPool[msg.sender].unLockTime - block.timestamp
                );
                rallReward =
                    userTokenPool[msg.sender].reward -
                    (userTokenPool[msg.sender].reward * rateFee) /
                    100;
            }
    ...
    function getFee(uint256 difTime) public view returns (uint256) {
        if (difTime == 0) {
            return 0;
        }
    ...
    function getTaskById(uint256 enemyId)
        public
        view
        returns (enemyInfo memory)
    {
    ...
    function isBack(uint256 tokenId, address sender)
        public
        view
        returns (bool)
    {
    ...
    {
        return (
            tokenDetail[tokenId].level,
            tokenDetail[tokenId].ce,
            tokenDetail[tokenId].xp,
            tokenDetail[tokenId].armor,
            tokenDetail[tokenId].luk,
            tokenDetail[tokenId].rgTime
        );
    }
    ...
    function editCardDetails(uint256 tokenId, address addr)
        public
        onlyRole(MINTER_ROLE)
    {
    ...
    function getUserTeamCards(address sender)
        public
        view
    ...
    function getUserBkCards(address sender)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function withdrawal(address addr, uint256 amount)
        public
        onlyOwner
        returns (bool)
    {
